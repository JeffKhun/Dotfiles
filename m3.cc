...
/**
 * To-Do: Boyer-Moore matching algorithm
 * @param text
 * @param pattern
 ...
 * To-Do:
 * Efficiently could be improved by only storing start and end numeric indices of
 * intermediate strings as STL pairs rather than repeatedly copying strings.
        ...
        if (s[i]=='\\') {   //Case 1: escape character
            ...
            while (s[++i]!='}') {
                if (s[i]>='0' && s[i]<='9')  //checks s[i] in {0,...9}
                    num=(num*10)+(s[i]-'0'); //calculates number of repetitions
                else return false;
            }
            if (s[i]!='}') return false;
            ...
            //r.reserve(r.capacity()*num);
            r*=num; //user-defined operator*=
            stk.push_back(r);
            r.clear();
...
int main() {
    ...
    unsigned char Match[]="abc[def[gh[i]{3}]{20000}jkl]{2000}mno";//"abc[def[ghi]{2}jkl]{2}mno";//"¢a¢bc";//"abc[def]{3}ghi[j]{4}z";//"abc[def]{3}ghi[j]{4}z";//"abc";
